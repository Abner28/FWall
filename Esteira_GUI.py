...
##############################################################################
# Which type of analysis? [1 for 'create wing' or 2 for 'Posprocessement']                                                                           
type_analysis = 0                                                          
##############################################################################
...
if type_analysis == 1:
    ...
    def posprocessement():
        coords = pd.read_csv("C:\\Users\Administrador\Desktop\Códigos_att\coords", sep = ",", header = None)
        y = coords.iloc[:,0]
        z = coords.iloc[:,1]
    ...
    def aerodynamics(y):
        ...
        circ =  pd.read_csv("C:\\Users\Administrador\Desktop\Códigos_att\circulation", sep = ",", header = None)
        G = circ.iloc[:,0]
    ...
    def bound_vortex(yp,zp,YP,ZP,G,y,z):
        ...
        if(wing_type == "Planar"):
            ...
            plt.colorbar(Q, cmap=plt.cm.jet)
            plt.plot(y[1*(Nelem+1):2*(Nelem+1)], z[1*(Nelem+1):2*(Nelem+1)], '-ok',markersize = 1.2, linewidth = .5, label = "Wing")
        ...
        plt.colorbar(Q, cmap=plt.cm.jet)
        plt.plot(y[0*(Nelem+1):1*(Nelem+1)], z[0*(Nelem+1):1*(Nelem+1)], '-ok',markersize = 1.2, linewidth = .5, label = "Wing")
        plt.legend(loc = (0.73,0.84), framealpha = 1.0)
